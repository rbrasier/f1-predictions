...
export function generatePostRaceEmailHTML(data: PostRaceEmailData): string {
  ...
  const adminReleaseSection = data.isAdminSample && data.adminReleaseUrl ? `
    <div style="background-color: #ffc107; border: 2px solid #ff9800; padding: 15px; margin-bottom: 20px; border-radius: 8px; text-align: center;">
      ...
      <p style="margin: 10px 0 0 0; font-size: 12px; color: #666;">This action runs in the background and will send to all users with results enabled.</p>
    </div>
  ` : '';
...
This action runs in the background and will send to all users with results enabled.
