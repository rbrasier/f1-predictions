...
export class F1DataTransformer {
  ...
  async importRaceResults(year: number, round: number): Promise<{
    ...
    try {
      ...
      const race = db.prepare('SELECT id FROM races WHERE round_number = ? AND season_id = (SELECT id FROM seasons WHERE year = ?)').get(round, year) as { id: number } | undefined;
      ...
      const existing = db.prepare('SELECT id FROM race_results WHERE race_id = ?').get(race.id) as { id: number } | undefined;
  ...
  async importSeasonStandings(year: number): Promise<{
    ...
    try {
      ...
      const season = db.prepare('SELECT id FROM seasons WHERE year = ?').get(year) as { id: number } | undefined;
      ...
      // Check if results already exist
      const existing = db.prepare('SELECT id FROM season_results WHERE season_id = ?').get(season.id) as { id: number } | undefined;
  ...
  private async findDriverIdByApiId(apiDriverId: string): Promise<number | null> {
    ...
    for (const name of possibleNames) {
      const driver = db.prepare('SELECT id FROM drivers WHERE name LIKE ?').get(`%${name}%`) as { id: number } | undefined;
      if (driver) {
        return driver.id;
      }
    }
    ...
    if (lastName) {
      const driver = db.prepare('SELECT id FROM drivers WHERE name LIKE ?').get(`%${lastName}%`) as { id: number } | undefined;
      if (driver) {
        return driver.id;
      }
    }
  ...
  private async findTeamIdByApiId(apiConstructorId: string): Promise<number | null> {
    ...
    for (const name of possibleNames) {
      const team = db.prepare('SELECT id FROM teams WHERE name LIKE ?').get(`%${name}%`) as { id: number } | undefined;
      if (team) {
        return team.id;
      }
    }
  ...
  private async findMidfieldHero(results: any[]): Promise<number | null> {
    ...
    const topTeams = db.prepare('SELECT id FROM teams WHERE is_top_four = 1').all() as { id: number }[];
    ...
    for (const result of results) {
      ...
      // Get driver's team
      const driver = db.prepare('SELECT team_id FROM drivers WHERE id = ?').get(driverId) as { team_id: number } | undefined;
      if (!driver) continue;
  ...
  private async isCrazyPredictionValidated(type: string, predictionId: number): Promise<boolean> {
    const validations = db.prepare('SELECT is_validated FROM crazy_prediction_validations WHERE prediction_type = ? AND prediction_id = ?').all(type, predictionId) as { is_validated: number }[];
  ...
  private async didCrazyPredictionHappen(type: string, predictionId: number): Promise<boolean> {
    const outcome = db.prepare('SELECT actually_happened FROM crazy_prediction_outcomes WHERE prediction_type = ? AND prediction_id = ?').get(type, predictionId) as { actually_happened: number } | undefined;
